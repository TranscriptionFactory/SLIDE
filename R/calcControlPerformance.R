...
## Make the real model and get the performance
    ...
    if (condition == 'auc'){
      ...
      perreal <- pROC::auc(response=as.matrix(y), predictor=as.matrix(yhat), levels = sort(unique(y[, 1])), quiet=T)
    ...
    for (i in 1:1000) {
      ...
      SumInt <- summary(lm(y ~ ., data = Data_fullRandom))
       ...
       if (condition == 'auc'){
        ...
        prerandom <- pROC::auc(response=as.matrix(y), predictor=as.matrix(yhat), levels = sort(unique(y[, 1])), quiet=T)
  ...
  cat("Getting performance for partially random model... \n")
    ...
    for (i in 1:1000) {
      ...
      if (condition == 'auc'){
        ...
        perPrandom <- pROC::auc(response=as.matrix(y), predictor=as.matrix(yhat),quiet=T)
      ...
      ggplot2::ggsave(P2, filename = paste0(out_path, "ControlPerformancePlot.png"))
        ...
        if (condition == 'auc'){
          lmod  <- lm(y~.,data=Data_real)
          yhat <- predict(lmod,Data_real[,-1,drop=F],type = 'response')
          perreal <- pROC::auc(response=as.matrix(y), predictor=as.matrix(yhat),quiet=T)
        }else if (condition == 'corr'){
          SumReal <- summary(lm(y ~ ., data = Data_real))
          perreal <- sqrt(SumReal$r.squared)
        }
        ...
        for (i in 1:1000) {
          ...
          if (condition == 'auc'){
            ...
            lmod  <- lm(y~.,data=Data_fullRandom)
            yhat <- predict(lmod,Data_fullRandom[,-1,drop=F],type = 'response')
            prerandom <- pROC::auc(response=as.matrix(y), predictor=as.matrix(yhat),quiet=T)
            Fullreport <- rbind(Fullreport, prerandom)
